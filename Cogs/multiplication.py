...
class Multiplication(commands.Cog):
    def __init__(self,Bot):
        self.Bot = Bot

    @commands.command()
    async def mul(self,ctx,number1 : float = None,number2 : float = None):
        if number1 is None:
            WARNembed=discord.Embed(title="**Error**",description=f"• **{ctx.message.author.mention},make sure you typed the command correctly!**\n• **Usage :** `-mul <number1> <number2>`",colour=discord.Color.red())
...
def setup(Bot):
    Bot.add_cog(Multiplication(Bot))
